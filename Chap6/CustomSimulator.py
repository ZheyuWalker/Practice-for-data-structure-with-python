...
# from ..Chap5.SQueue import SQueue
# python -m ../Chap6/Simulator.py
...
class Simulation:
    ...
    def run(self):
        # Simulating until the event queue is empty
        while not self._eventq.is_empty():
            event = self._eventq.dequeue()
            self._time = event.time()
            if self._time > self._duration:
                break
            # simulation current event and may generate new event
            event.run()    
...
class Event:
    ...
    def __le__(self, other_event):
        return self._ctime <= other_enent._ctime
